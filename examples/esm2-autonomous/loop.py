...
def frozen_environment(run, manifest):
    ...
    return {**os.environ, 'PYTHONPATH': str(frozen), 'ESM2_REPOSITORY_ROOT': str(repository)}
...
def agent_environment(manifest):
    ...
    for role in ('worker', 'supervisor'):
        environment['ESM2_' + role.upper() + '_BACKEND'] = backends.get(role, 'opencode')
        if models.get(role):
            environment['ESM2_' + role.upper() + '_MODEL'] = models[role]
    return environment
...
if __name__ == '__main__' and '--controller' in sys.argv:
    ...
    os.environ.update({k: environment[k] for k in ('PYTHONPATH', 'ESM2_REPOSITORY_ROOT')})
...
WEIGHT_SHA256 = model_spec("esm2")["files"]["model.safetensors"]["sha256"]
from cluster import activated, identity as digest, load as load_cluster, ssh, upload, remote, reference_job

HERE = Path(__file__).resolve().parent
CORPUS_HASH = '369c475fa78f1aad39963eee3fc43344c62be766793ed529e12fc1fdb4326a9e'
...
def tool_name(run):
    ...
    return 'esm2_' + hashlib.sha256(str(Path(run).resolve()).encode()).hexdigest()[:16]
...
def supervisor_facts(workspace, run, config, model="esm2"):
    ...
    for path in sorted((Path(run) / 'jobs').glob('*/request.json'), key=lambda p: p.stat().st_mtime):
        ...
        item = {'id': path.parent.name, 'kind': request.get('kind', 'evaluate'),
                'stage': request.get('stage', 'experiment'), 'model': request.get('model', 'esm2'),
    ...
    return {'objective': 'Deliver ONE production-level reusable Esm2 TDT TTNN source packaged in '
...
def seed_workspace(workspace, seed_run, model, seed_job=None):
    ...
    if (sorted(manifest.get('seed_files', [])) != sorted(trusted)
            and manifest.get('seed_policy') != 'fresh-public-esm2'):
        raise ValueError('seed must be a fresh public-source Esm2 campaign')
...
def resume_workspace(workspace, previous_run, model, backends, note=''):
    ...
    manifest = json.loads((root / 'manifest.json').read_text())
    if manifest.get('seed_policy') != 'fresh-public-esm2':
        raise ValueError('resume source must be a fresh public-source Esm2 campaign')
    ...
    header = ('# Resumed campaign\n\n'
        ...
        'esm2_status(job_id=...) and esm2_log still read them as historical receipts, but no '
...
def prepare(run, config):
    ...
    weights = endpoint.get('weights_by_model', {'esm2': endpoint.get('weights')})
...
def validate_reference_archive(path, corpus, stage, model):
    ...
    with tarfile.open(fileobj=io.BytesIO(data)) as archive:
        ...
        for key in ('stage', 'corpus_sha256', 'max_residues', 'gates', 'timed_repeats'):
...
def reference(run_dir, stage, config, model="esm2"):
    model_spec(model)
...
def experiment(run_dir, job, request, config):
    ...
    try:
        ...
        result['model'] = request.get('model', 'esm2')
...
@ChiaFunction(resources={'reference': 1}, max_retries=0)
def precision_study(run_dir, job, config, model="esm2"):
...
class PortTools(ChiaTool):
    ...
    def start(self, request):
        ...
        request = {**request, 'model': request.get('model', getattr(self, 'model', 'esm2')),
                   'cluster_sha256': digest(config)}
        input_stage = stage or request.get('public_input_stage')
        if input_stage:
            request['suite_definition_sha256'] = suite_identity(self.run_dir, request.get('model', 'esm2'), input_stage)
        for path in (self.run_dir / 'jobs').glob('*/request.json'):
            ...
            if (stage and old.get('stage') == stage and old['code_sha256'] == code_hash(files)
                    ...
                    and old.get('model', 'esm2') == request.get('model', 'esm2')
    ...
    def run(self, command: str, timeout_seconds: int = 120, public_input_stage: str = '', model: str = '') -> dict:
        ...
        model = model or getattr(self, 'model', 'esm2')
        model_spec(model)
        ...
        for path in visible:
            ...
            if result_file.exists():
                ...
                if not job_id and request.get('stage') == 'bringup' and result.get('verdict', {}).get('passed'):
                    baseline = (self.run_dir / 'baselines' / request.get('model', 'esm2')
                    ...
                    if not baseline.exists():
                        write_json(baseline, {'job': path.parent.name, 'code_sha256': request['code_sha256'],
                                             ...
                                             'model': request.get('model', 'esm2'), 'suite_sha256': result['verdict']['suite_sha256'],
            ...
            if compact or job_id:
                ...
                item['details'] = ({'tool': 'esm2_log', 'job_id': path.parent.name,
                                   ...
                                   {'tool': 'esm2_status', 'compact': False, 'job_id': path.parent.name,
    ...
    def submit(self, precision: str = 'bf16', model: str = '') -> dict:
        ...
        for path in requests:
            ...
            if (req.get('kind') == 'evaluate' and req.get('stage') == 'bringup'
                    ...
                    and req.get('model', 'esm2') == model and req.get('precision') == precision
...
def main():
    ...
    parser.add_argument('--model', default='esm2')
    ...
    if not args.controller:
        ...
        workspace = Path(tempfile.mkdtemp(prefix='esm2-fresh-'))
    ...
    if args.model not in config['tt'].get('weights_by_model', {'esm2': config['tt'].get('weights')}):
    ...
    ray.init(num_cpus=8, include_dashboard=False, _node_ip_address='127.0.0.1',
             runtime_env={'env_vars': {'PYTHONPATH': str(run / 'harness'),
                                      'ESM2_REPOSITORY_ROOT': manifest['repository_root'],
    ...
    try:
        ...
        preflight = get(worker_node.chia_remote(str(workspace), str(run), [tools], 0,
            'Infrastructure check only: list files, read CONTRACT.md, confirm esm2_read ../outside is rejected. '
        ...
        while time.monotonic() < end:
            ...
            previous = get(worker_node.chia_remote(str(workspace), str(run), [tools], turn,
                review['decision']['instruction'] + '\nContinue from your saved STATE.md. Call esm2_status(compact=True) before repeating tests.'))
        ...
        if (run / 'submission.json').exists():
            ...
            result = get(experiment.chia_remote(str(run), 'final', {'kind':'evaluate','stage':'full',
                'files':submission['files'], 'precision':submission['precision'], 'model':submission.get('model', 'esm2')}, frozen))
