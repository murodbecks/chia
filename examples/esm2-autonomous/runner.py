...
def configure(config):
    ...
    if config.get('weights'):
        weights.setdefault('esm2', config['weights'])
    if not weights:
    ...
    WEIGHTS_BY_MODEL, WEIGHTS = weights, weights.get('esm2')
...
def sandbox(root, job, pci, node, model='esm2'):
...
def main(root, request):
    ...
    if input_stage:
        ...
        if (reference / 'suite.json').exists():
            ...
            if prepared.get('model_key', 'esm2') != model:
