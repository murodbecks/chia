...
def reference_execution():
    mode = os.getenv("ESM2_REFERENCE_EXECUTION", "slurm")
...
def make_suite(corpus, stage, model="esm2"):
    ...
    if stage == "smoke":
        ...
        pick = corpus["selection"]["bringup"]
        add("batch4", [pick["batch0"], pick["batch1"], corpus["selection"]["full"][0], corpus["selection"]["full"][-1]], quality=True)
        add("batch4-reverse", [corpus["selection"]["full"][-1], corpus["selection"]["full"][0], pick["batch1"], pick["batch0"]],
            compare_to="batch4", compare_rows=[3, 2, 1, 0])
        add("revisit", [pick["batch0"], pick["batch1"]], compare_to="batch", compare_rows=[0, 1])
        add("single-residue", ["syn-single"])
