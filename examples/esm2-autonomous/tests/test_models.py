...
class ModelTests(unittest.TestCase):
    ...
    def test_real_registry_pins_esm2_safetensors(self):
        spec = models.model_spec("esm2")
        self.assertEqual(spec["repo_id"], "facebook/esm2_t33_650M_UR50D")
        ...
        self.assertEqual(arch["hidden_size"], 1280)
        self.assertEqual(arch["num_hidden_layers"], 33)
        self.assertEqual(arch["num_attention_heads"], 20)
        self.assertEqual(arch["vocab_size"], 33)
        self.assertEqual(arch["position_embedding_type"], "rotary")
        self.assertEqual(len(spec["files"]["model.safetensors"]["sha256"]), 64)
        for extra in ("vocab.txt", "special_tokens_map.json"):
            self.assertIn(extra, spec["files"])
        with self.assertRaisesRegex(ValueError, "Unknown model"):
            models.model_spec("esm2_t33")
