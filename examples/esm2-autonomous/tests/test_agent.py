...
def make_agent(web=True, repository='/workspace/original-repository', **kwargs):
    return agent.IsolatedOpenCode(model=agent.MODEL, agent_name='esm2-worker',
...
class AgentTests(unittest.TestCase):
    def setUp(self):
        self.environment = patch.dict(os.environ, {'ESM2_REPOSITORY_ROOT': '/workspace/original-repository'})
    ...
    def test_isolation_config_denies_host_writes_exec_and_secrets(self):
        tool = SimpleNamespace(name='esm2_tools', hostname='127.0.0.1', port=8000)
        ...
        self.assertEqual(config["mcp"]["esm2_tools"],
                         {"type": "remote", "url": "http://127.0.0.1:8000/esm2_tools/mcp",
                          "enabled": True})
...
class FakeMCPTool:
    ...
    mcp = SimpleNamespace(_tool_manager=SimpleNamespace(list_tools=lambda: [
        SimpleNamespace(name='esm2_run'), SimpleNamespace(name='esm2_write')]))
...
class ClaudeBackendTests(unittest.TestCase):
    ...
    def test_role_selection_defaults_overrides_and_legacy_model(self):
        ...
        with patch.dict(os.environ, {'ESM2_AGENT_MODEL': 'zai/legacy'}, clear=True):
            self.assertEqual(agent.role_selection('worker'), ('opencode', 'zai/legacy'))
        with patch.dict(os.environ, {'ESM2_AGENT_MODEL': 'zai/legacy', 'ESM2_WORKER_BACKEND': 'claude',
                                     'ESM2_SUPERVISOR_BACKEND': 'opencode'}, clear=True):
        ...
        with patch.dict(os.environ, {'ESM2_SUPERVISOR_BACKEND': 'claude',
                                     'ESM2_SUPERVISOR_MODEL': 'claude-small'}, clear=True):
            self.assertEqual(agent.role_selection('supervisor'), ('claude', 'claude-small'))
        with patch.dict(os.environ, {'ESM2_WORKER_BACKEND': 'codex'}, clear=True), \
    ...
    def test_claude_command_denies_host_writes_exec_and_secrets(self):
        ...
        self.assertEqual(command[command.index('--allowedTools') + 1],
                         'mcp__esm2_tools__esm2_run,mcp__esm2_tools__esm2_write')
        mcp = json.loads(Path(command[command.index('--mcp-config') + 1]).read_text())
        self.assertEqual(mcp, {'mcpServers': {'esm2_tools': {
            'type': 'http', 'url': 'http://127.0.0.1:8000/esm2_tools/mcp'}}})
    ...
    def test_query_uses_claude_and_records_backend(self):
        ...
        with tempfile.TemporaryDirectory() as root, \
             patch.dict(os.environ, {'ESM2_WORKER_BACKEND': 'claude', 'ESM2_WORKER_MODEL': 'claude-test'}), \
    ...
    def test_claude_timeout_and_usage_limit_are_categorized(self):
        with tempfile.TemporaryDirectory() as root, \
             patch.dict(os.environ, {'ESM2_WORKER_BACKEND': 'claude'}):
