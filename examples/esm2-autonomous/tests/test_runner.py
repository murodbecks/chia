...
class RunnerTests(unittest.TestCase):
    ...
    def config(self, root):
        return {'metal': '/deployment/tt-metal', 'python': '/deployment/python/bin/python',
                'weights': '/models/esm2/model.safetensors', 'health_dir': str(root / 'health'),
    ...
    def test_runtime_configuration_has_no_host_or_user_dependency(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(runner.WEIGHTS, Path('/models/esm2/model.safetensors'))
            index = args.index('/deployment/python')
            self.assertEqual(args[index - 1], '--ro-bind')
            self.assertNotIn('/models/esm2', args)
            self.assertIn('/models/esm2/model.safetensors', args)
    ...
    def test_sharded_mount_exposes_only_selected_manifest_files(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            checkpoint = root / 'weights-esm2'
            checkpoint.mkdir()
            spec = runner.model_spec('esm2')
            ...
            runner.configure({**self.config(root), 'weights_by_model': {'esm2': str(checkpoint)}})
            args = runner.sandbox(root, root / 'job', '0000:02:00.0', '7', 'esm2')
            self.assertEqual(runner.checkpoint('esm2'), (checkpoint, '/weights'))
            self.assertNotIn(str(checkpoint), args)
            self.assertNotIn(str(checkpoint / 'earlier-private-backend.py'), args)
            self.assertNotIn('/models/esm2/model.safetensors', args)
    ...
    def test_model_mapping_rejects_unknown_models_and_unconfigured_keys(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(runner.checkpoint('esm2')[1], '/weights/model.safetensors')

    def test_evaluation_uses_model_specific_inputs_and_excludes_extra_files(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / 'harness').mkdir()
            config = {**self.config(root), 'weights_by_model': {'esm2': '/models/large/model.safetensors'}}
            (root / 'harness/runtime.json').write_text(json.dumps(config))
            inputs = root / 'references/esm2/smoke/input'
            inputs.mkdir(parents=True)
            (inputs / 'suite.json').write_text(json.dumps({'model_key': 'esm2'}))
            ...
            with patch.object(runner, 'cards', return_value={'0000:02:00.0': '7'}), \
                ...
                for kind in ('evaluate', 'run'):
                    with self.subTest(kind=kind):
                        (inputs / 'suite.json').write_text(json.dumps({'model_key': 'esm2'}))
                        request = {'kind': kind, 'job': 'larger-' + kind, 'model': 'esm2'}
                        ...
                        self.assertNotIn('/models/esm2/model.safetensors', captured[-1])
    ...
    def test_candidate_command_passes_validated_precision_and_weight_filename(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (root / 'references/esm2/smoke/input').mkdir(parents=True)
            captured = []
            def execute(args, job, *unused):
                captured.append(args)
    ...
    def test_nondefault_model_preflight_and_hashing_timeout_do_not_require_anchor(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            config = self.config(root)
            del config['weights']
            config['weights_by_model'] = {'esm2': '/models/large/model.safetensors'}
            (root / 'harness/runtime.json').write_text(json.dumps(config))
            (root / 'references/esm2/smoke/input').mkdir(parents=True)
            ...
            with patch.object(runner, 'cards', return_value={'0000:02:00.0': '7'}), \
                ...
                result = runner.main(root, {'kind': 'preflight', 'pci': '0000:02:00.0', 'job': 'preflight', 'model': 'esm2'})
                ...
                result = runner.main(root, {'kind': 'evaluate', 'stage': 'smoke', 'job': 'slow-hash', 'model': 'esm2'})
    ...
    def test_full_deadline_changes_only_full_evaluation_and_ignores_candidate_override(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for stage in ('smoke', 'bringup', 'full'):
                (root / 'references' / 'esm2' / stage / 'input').mkdir(parents=True)
            captured = []
    ...
    def test_profile_run_mounts_only_readonly_public_inputs_with_normal_timeout(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (root / 'harness').mkdir()
            (root / 'harness/runtime.json').write_text(json.dumps(self.config(root)))
            public = root / 'references/esm2/smoke/input'
            public.mkdir(parents=True)
            for name, content in (('suite.json', '{"model_key":"esm2","stage":"smoke"}'),
