...
MODEL = os.environ.get("ESM2_AGENT_MODEL", DEFAULT_MODELS["opencode"])
...
WORKER_TIMEOUT_SECONDS = int(os.environ.get("ESM2_WORKER_TIMEOUT_SECONDS", "1800"))
SUPERVISOR_TIMEOUT_SECONDS = int(os.environ.get("ESM2_SUPERVISOR_TIMEOUT_SECONDS", "300"))
...
def repository_root():
    root = Path(os.environ.get('ESM2_REPOSITORY_ROOT', ROOT))
...
def role_selection(role):
    ...
    backend = (os.environ.get(f"ESM2_{key}_BACKEND")
               or os.environ.get("ESM2_AGENT_BACKEND") or "opencode")
    if backend not in BACKENDS:
        raise ValueError(f"unknown agent backend {backend!r}; choose {', '.join(BACKENDS)}")
    legacy = os.environ.get("ESM2_AGENT_MODEL") if backend == "opencode" else None
    return backend, os.environ.get(f"ESM2_{key}_MODEL") or legacy or DEFAULT_MODELS[backend]
...
def agent_name(role):
    return "esm2-worker" if role == "worker" else "esm2-supervisor"
...
def query(workspace, run_dir, role, turn, prompt, tools, web, timeout):
    ...
    if backend == "claude":
        agent = IsolatedClaude(model=model, work_dir=workspace, timeout_seconds=timeout,
            retries=1, logging_name="esm2-" + role, log_dir=str(logs), web=web,
        ...
        agent = IsolatedOpenCode(model=model, agent_name=agent_name(role),
            ...
            logging_name="esm2-" + role, log_dir=str(logs), web=web,
...
def worker_node(workspace, run_dir, tools, turn, prompt):
    instructions = ("Port AND optimize amazon/esm2-2 through the supplied CHIA tools, producing a "
...
def supervisor_node(workspace, run_dir, turn, summary):
    instructions = ("You supervise the objective stated in the observation: a correct AND optimized, "
        "production-level ESM-2 650M port packaged in tt-metal style, not just first bringup. "
        ...
        "Follow selected_model (esm2 for a fresh campaign), use tiny independent correctness checks, then broader "
    ...
    attempts = []
    with tempfile.TemporaryDirectory(prefix="esm2-supervisor-") as directory:
