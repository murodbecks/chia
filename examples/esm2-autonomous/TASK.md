...
3. backend.py via `esm2_run` experiments; pinned checkpoint under `/weights`;
   tokenization is deterministic CPU policy.
4. `esm2_evaluate(stage="smoke")` → FP32 oracle check → `bringup` → profile →
...
Only `esm2_write` persists edits. `esm2_run/evaluate/status/log/submit` are the
