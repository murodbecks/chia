...
def model_identity(name="parakeet"):
...
def suite_model(spec):
    name = spec.get("model_key", "parakeet")
...
def verify_weights(path, name):
    """Single safetensors checkpoint or an official checkpoint directory; hash bytes, never execute."""
...
def reference_execution():
    mode = os.getenv("PARAKEET_REFERENCE_EXECUTION", "slurm")
