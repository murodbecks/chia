...
set -euo pipefail
reference_root="${1:-$HOME/parakeet-autonomous}"
...
"$reference_python" -m pip install --index-url https://pypi.org/simple transformers==5.17.0 numpy==2.4.4 soundfile==0.13.1 sentencepiece==0.2.1 --report "$reference_root/reference-install.json"
"$reference_python" -m pip check
"$reference_python" -m pip freeze > "$reference_root/requirements.lock.txt"
"$reference_python" - <<'PY'
import torch
import transformers, soundfile  # noqa: F401  (import guards)
