# Lessons from the NLLB and Chronos-2 campaigns (binding rules)

Distilled from the NLLB-600M and Chronos-2 Blackhole campaigns (Sep 2026) and
the failed first NLLB attempt before them. Each lesson is a rule for this campaign;
...
6. **Truncated logs are not a reason to rerun.** Page the saved log with
   `parakeet_log(job_id, offset_bytes=…, limit_bytes=…)` and continue from
   `next_offset_bytes`.
...
8. **Pending jobs are polled, not duplicated.** Use `parakeet_status` and do
...
13. **Numerics gates match the task.** NLLB used exact token equality end to end;
    Chronos-2 used NRMSE (≤0.04) on float forecasts. Parakeet uses BOTH: encoder
    NRMSE ≤0.04 and exact greedy-token match vs the FP32 oracle. If bf16 argmax
    flips tokens in the decode loop, that is a real finding — diagnose with a
    per-step FP32 exception or HiFi matmul on the joint network, don't widen gates.
14. **Autoregressive decode is the TT overhead trap.** Every emitted symbol can
    cost a host round trip. Trace capture + multi-symbol TDT steps are the
    intended weapons; profile tokens/second, not just per-case latency.
15. **Bounded scope beats a bloated workspace.** Attempt #1 died in a
    cluttered, growing workspace. Keep the workspace minimal, no recursive note
    archives, no embedded logs in STATE.md.
16. **Honest envelope reporting.** Clips beyond the frozen envelope, streaming/
    chunked inference and multilingual coverage beyond the corpus are reported
    as unsupported, not implied. "Production-level" means the tt-metal package,
    tests and docs quality — not certification claims.

17. **Shared machine discipline.** tt-blackhole-01 has other users: never
    touch files outside this campaign's workspace, never reset cards, and keep
    host load bounded (the NLLB 'never reset cards' rule, stricter here).
