...
def frozen_environment(run, manifest):
    ...
    return {**os.environ, 'PYTHONPATH': str(frozen), 'PARAKEET_REPOSITORY_ROOT': str(repository)}
...
def agent_environment(manifest):
    ...
    for role in ('worker', 'supervisor'):
        environment['PARAKEET_' + role.upper() + '_BACKEND'] = backends.get(role, 'opencode')
        if models.get(role):
            environment['PARAKEET_' + role.upper() + '_MODEL'] = models[role]
    return environment
...
if __name__ == '__main__' and '--controller' in sys.argv:
    ...
    os.environ.update({k: environment[k] for k in ('PYTHONPATH', 'PARAKEET_REPOSITORY_ROOT')})
...
WEIGHT_SHA256 = model_spec("parakeet")["files"]["model.safetensors"]["sha256"]
from cluster import activated, identity as digest, load as load_cluster, ssh, upload, remote, reference_job

HERE = Path(__file__).resolve().parent
CORPUS_HASH = '62fb9c517719931999ccd72b9c097e889e843aaf8345567f62f16e95e8248d02'
...
def tool_name(run):
    ...
    return 'parakeet_' + hashlib.sha256(str(Path(run).resolve()).encode()).hexdigest()[:16]
...
def supervisor_facts(workspace, run, config, model="parakeet"):
    ...
    for path in sorted((Path(run) / 'jobs').glob('*/request.json'), key=lambda p: p.stat().st_mtime):
        ...
        item = {'id': path.parent.name, 'kind': request.get('kind', 'evaluate'),
                'stage': request.get('stage', 'experiment'), 'model': request.get('model', 'parakeet'),
    ...
    return {'objective': 'Deliver ONE production-level reusable Parakeet TDT TTNN source packaged in '
                         'tt-metal style (tt/, demo/, reference/, tests/, benchmarks/, docs/ with SPDX '
                         'headers) suitable for an upstream tt-metal PR. Establish measured baselines '
                         'FIRST and preserve them (PyTorch on the TT host; the CUDA reference timings on the A100 '
                         'are the NVIDIA comparison target); pass smoke, then bringup against the FP32 oracle; profile and optimize against '
                         ...
                         'diagnosis blocks final evaluation. Follow LESSONS.md. ',
...
def seed_workspace(workspace, seed_run, model, seed_job=None):
    ...
    trusted = ('CONTRACT.md', 'TT_GUIDE.md', 'TASK.md', 'BENCHMARK.md', 'LESSONS.md')
    if (sorted(manifest.get('seed_files', [])) != sorted(trusted)
            and manifest.get('seed_policy') != 'fresh-public-parakeet'):
        raise ValueError('seed must be a fresh public-source Parakeet campaign')
...
def same_evaluator(run):
    ...
    return frozen.is_file() and frozen.read_bytes() == (HERE / 'evaluate.py').read_bytes()
...
def resume_workspace(workspace, previous_run, model, backends, note=''):
    ...
    manifest = json.loads((root / 'manifest.json').read_text())
    if manifest.get('seed_policy') != 'fresh-public-parakeet':
        raise ValueError('resume source must be a fresh public-source Parakeet campaign')
    ...
    header = ('# Resumed campaign\n\n'
        ...
        'parakeet_status(job_id=...) and parakeet_log still read them as historical receipts, but no '
...
def prepare(run, config):
    ...
    weights = endpoint.get('weights_by_model', {'parakeet': endpoint.get('weights')})
...
def reference(run_dir, stage, config, model="parakeet"):
    model_spec(model)
...
def experiment(run_dir, job, request, config):
    ...
    try:
        ...
        result['model'] = request.get('model', 'parakeet')
...
@ChiaFunction(resources={'reference': 1}, max_retries=0)
def precision_study(run_dir, job, config, model="parakeet"):
...
class PortTools(ChiaTool):
    ...
    def write(self, path: str, content: str) -> dict:
        if Path(path).as_posix() in ('CONTRACT.md', 'TT_GUIDE.md', 'TASK.md', 'BENCHMARK.md', 'LESSONS.md') or (self.run_dir / 'submission.json').exists():
    ...
    def start(self, request):
        ...
        request = {**request, 'model': request.get('model', getattr(self, 'model', 'parakeet')),
                   'cluster_sha256': digest(config)}
        input_stage = stage or request.get('public_input_stage')
        if input_stage:
            request['suite_definition_sha256'] = suite_identity(self.run_dir, request.get('model', 'parakeet'), input_stage)
        for path in (self.run_dir / 'jobs').glob('*/request.json'):
            ...
            if (stage and old.get('stage') == stage and old['code_sha256'] == code_hash(files)
                    ...
                    and old.get('model', 'parakeet') == request.get('model', 'parakeet')
    ...
    def run(self, command: str, timeout_seconds: int = 120, public_input_stage: str = '', model: str = '') -> dict:
        ...
        model = model or getattr(self, 'model', 'parakeet')
        model_spec(model)
        ...
        for path in visible:
            ...
            if result_file.exists():
                ...
                if not job_id and request.get('stage') == 'bringup' and result.get('verdict', {}).get('passed'):
                    baseline = (self.run_dir / 'baselines' / request.get('model', 'parakeet')
                    ...
                    if not baseline.exists():
                        write_json(baseline, {'job': path.parent.name, 'code_sha256': request['code_sha256'],
                                             ...
                                             'model': request.get('model', 'parakeet'), 'suite_sha256': result['verdict']['suite_sha256'],
            ...
            if compact or job_id:
                ...
                item['details'] = ({'tool': 'parakeet_log', 'job_id': path.parent.name,
                                   ...
                                   {'tool': 'parakeet_status', 'compact': False, 'job_id': path.parent.name,
    ...
    def submit(self, precision: str = 'bf16', model: str = '') -> dict:
        ...
        for path in requests:
            ...
            if (req.get('kind') == 'evaluate' and req.get('stage') == 'bringup'
                    ...
                    and req.get('model', 'parakeet') == model and req.get('precision') == precision
...
def main():
    ...
    if not args.controller:
        ...
        workspace = Path(tempfile.mkdtemp(prefix='parakeet-fresh-'))
        for name in ('CONTRACT.md', 'TT_GUIDE.md', 'TASK.md', 'BENCHMARK.md', 'LESSONS.md'):
    ...
    if args.model not in config['tt'].get('weights_by_model', {'parakeet': config['tt'].get('weights')}):
    ...
    ray.init(num_cpus=8, include_dashboard=False, _node_ip_address='127.0.0.1',
             runtime_env={'env_vars': {'PYTHONPATH': str(run / 'harness'),
                                      'PARAKEET_REPOSITORY_ROOT': manifest['repository_root'],
    ...
    try:
        ...
        preflight = get(worker_node.chia_remote(str(workspace), str(run), [tools], 0,
            'Infrastructure check only: list files, read CONTRACT.md, confirm parakeet_read ../outside is rejected. '
        ...
        while time.monotonic() < end:
            ...
            previous = get(worker_node.chia_remote(str(workspace), str(run), [tools], turn,
                review['decision']['instruction'] + '\nContinue from your saved STATE.md. Call parakeet_status(compact=True) before repeating tests.'
        ...
        if (run / 'submission.json').exists():
            ...
            result = get(experiment.chia_remote(str(run), 'final', {'kind':'evaluate','stage':'full',
                'files':submission['files'], 'precision':submission['precision'], 'model':submission.get('model', 'parakeet')}, frozen))
