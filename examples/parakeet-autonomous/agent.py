...
MODEL = os.environ.get("PARAKEET_AGENT_MODEL", DEFAULT_MODELS["opencode"])
...
WORKER_TIMEOUT_SECONDS = int(os.environ.get("PARAKEET_WORKER_TIMEOUT_SECONDS", "1800"))
SUPERVISOR_TIMEOUT_SECONDS = int(os.environ.get("PARAKEET_SUPERVISOR_TIMEOUT_SECONDS", "300"))
...
def repository_root():
    root = Path(os.environ.get('PARAKEET_REPOSITORY_ROOT', ROOT))
...
def role_selection(role):
    ...
    backend = (os.environ.get(f"PARAKEET_{key}_BACKEND")
               or os.environ.get("PARAKEET_AGENT_BACKEND") or "opencode")
    if backend not in BACKENDS:
        raise ValueError(f"unknown agent backend {backend!r}; choose {', '.join(BACKENDS)}")
    legacy = os.environ.get("PARAKEET_AGENT_MODEL") if backend == "opencode" else None
    return backend, os.environ.get(f"PARAKEET_{key}_MODEL") or legacy or DEFAULT_MODELS[backend]
...
def agent_name(role):
    return "parakeet-worker" if role == "worker" else "parakeet-supervisor"
...
def query(workspace, run_dir, role, turn, prompt, tools, web, timeout):
    ...
    if backend == "claude":
        agent = IsolatedClaude(model=model, work_dir=workspace, timeout_seconds=timeout,
            retries=1, logging_name="parakeet-" + role, log_dir=str(logs), web=web,
        ...
        agent = IsolatedOpenCode(model=model, agent_name=agent_name(role),
            ...
            logging_name="parakeet-" + role, log_dir=str(logs), web=web,
...
def worker_node(workspace, run_dir, tools, turn, prompt):
    instructions = ("Port AND optimize nvidia/parakeet-tdt-0.6b-v3 through the supplied CHIA tools, producing a "
        ...
        "tests. Read LESSONS.md early; those mistakes are expensive to repeat. "
        ...
        "Read TASK.md, CONTRACT.md, TT_GUIDE.md, BENCHMARK.md and LESSONS.md first; read STATE.md when resuming. "
...
def supervisor_node(workspace, run_dir, turn, summary):
    instructions = ("You supervise the objective stated in the observation: a correct AND optimized, "
        "production-level Parakeet TDT port packaged in tt-metal style, not just first bringup. "
        ...
        "Follow selected_model (parakeet for a fresh campaign), use tiny independent correctness checks, then broader "
    ...
    attempts = []
    with tempfile.TemporaryDirectory(prefix="parakeet-supervisor-") as directory:
