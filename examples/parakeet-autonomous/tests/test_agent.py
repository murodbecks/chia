...
def make_agent(web=True, repository='/workspace/original-repository', **kwargs):
    return agent.IsolatedOpenCode(model=agent.MODEL, agent_name='parakeet-worker',
...
class AgentTests(unittest.TestCase):
    def setUp(self):
        self.environment = patch.dict(os.environ, {'PARAKEET_REPOSITORY_ROOT': '/workspace/original-repository'})
...
class FakeMCPTool:
    ...
    mcp = SimpleNamespace(_tool_manager=SimpleNamespace(list_tools=lambda: [
        SimpleNamespace(name='parakeet_run'), SimpleNamespace(name='parakeet_write')]))
...
class ClaudeBackendTests(unittest.TestCase):
    ...
    def test_role_selection_defaults_overrides_and_legacy_model(self):
        ...
        with patch.dict(os.environ, {'PARAKEET_AGENT_MODEL': 'zai/legacy'}, clear=True):
            self.assertEqual(agent.role_selection('worker'), ('opencode', 'zai/legacy'))
        with patch.dict(os.environ, {'PARAKEET_AGENT_MODEL': 'zai/legacy', 'PARAKEET_WORKER_BACKEND': 'claude',
                                     'PARAKEET_SUPERVISOR_BACKEND': 'opencode'}, clear=True):
        ...
        with patch.dict(os.environ, {'PARAKEET_SUPERVISOR_BACKEND': 'claude',
                                     'PARAKEET_SUPERVISOR_MODEL': 'claude-small'}, clear=True):
            self.assertEqual(agent.role_selection('supervisor'), ('claude', 'claude-small'))
        with patch.dict(os.environ, {'PARAKEET_WORKER_BACKEND': 'codex'}, clear=True), \
    ...
    def test_claude_command_denies_host_writes_exec_and_secrets(self):
        ...
        self.assertEqual(command[command.index('--allowedTools') + 1],
                         'mcp__parakeet_tools__parakeet_run,mcp__parakeet_tools__parakeet_write')
    ...
    def test_query_uses_claude_and_records_backend(self):
        ...
        with tempfile.TemporaryDirectory() as root, \
             patch.dict(os.environ, {'PARAKEET_WORKER_BACKEND': 'claude', 'PARAKEET_WORKER_MODEL': 'claude-test'}), \
    ...
    def test_claude_timeout_and_usage_limit_are_categorized(self):
        with tempfile.TemporaryDirectory() as root, \
             patch.dict(os.environ, {'PARAKEET_WORKER_BACKEND': 'claude'}):
