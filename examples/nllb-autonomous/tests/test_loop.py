...
class LoopTests(unittest.TestCase):
    ...
    def test_status_exact_old_job_keeps_complete_gate_and_is_read_only(self):
        ...
        request = {'kind': 'evaluate', 'stage': 'bringup', 'model': '600m',
    ...
    def test_status_exact_pending_is_local_and_preserves_request(self):
        request = {'kind': 'reference-study', 'model': '600m', 'cluster_sha256': 'missing'}
    ...
    def test_status_exact_historical_dotted_reference_id(self):
        job_id = 'gpu-precision-1.3b-distilled-20260921T1020'
        request = {'kind': 'reference-study', 'model': '1.3b-distilled'}
    ...
    def test_timeout_recovery_requires_real_probe_and_cannot_repeat(self):
        ...
        with patch.object(loop, 'remote') as remote:
            ...
            self.assertEqual(loop.recover_timed_out_card(
                self.tools.run_dir, self.config, health, '1.3b-distilled'), health)
    ...
    def test_timeout_recovery_refuses_pending_memory_failure_or_wrong_identity(self):
        ...
        with patch.object(loop, 'remote') as remote:
            for req, res in variants:
                ...
                self.assertEqual(loop.recover_timed_out_card(
                    self.tools.run_dir, self.config, health, '600m'), health)
            write_json(job / 'request.json', request)
            write_json(job / 'result.json', result)
    ...
    def test_timeout_recovery_preserves_failed_or_ambiguous_probe(self):
        ...
        with patch.object(loop, 'remote', side_effect=TimeoutError('transport ambiguous')):
            with self.assertRaises(TimeoutError):
                loop.recover_timed_out_card(self.tools.run_dir, self.config, health, '600m')
        ...
        with patch.object(loop, 'remote', side_effect=[{'returncode': 124, 'timeout': True}, health]):
            self.assertEqual(loop.recover_timed_out_card(
                self.tools.run_dir, replacement, health, '600m'), health)
    ...
    def test_compact_status_preserves_failure_quality_identity_and_legacy_response(self):
        ...
        request = {'kind': 'evaluate', 'stage': 'smoke', 'model': '600m', 'precision': 'bf16',
        ...
        result = {'returncode': 125, 'failure': 'host_memory_limit', 'timeout': False,
                  ...
                  'model': '600m', 'precision': 'bf16', 'pci': '0000:41:00.0',
        ...
        self.assertEqual(compact['details']['tool'], 'nllb_log')
    ...
    def test_compact_status_keeps_old_pending_and_reference_study_details(self):
        ...
        write_json(run / 'jobs/study/request.json', {'kind': 'reference-study', 'model': '600m'})
        ...
        for job in ('study', 'pending-reference'):
            self.assertEqual(compact[job]['details']['tool'], 'nllb_status')
    ...
    def test_run_punctuation_cannot_create_nested_mcp_config_keys(self):
        from types import SimpleNamespace
        from agent import IsolatedCodex
        name = loop.tool_name(self.tools.run_dir / 'model-1.3b with spaces')
        self.assertRegex(name, r'^nllb_[a-f0-9]{16}$')
        tool = SimpleNamespace(name=name, hostname='127.0.0.1', port=8002)
        args = object.__new__(IsolatedCodex)._mcp_config_args([tool])
        keys = [arg.split('=', 1)[0] for arg in args if arg != '-c']
        self.assertEqual(keys, [f'mcp_servers.{name}.{key}' for key in ('url', 'default_tools_approval_mode', 'required')])
    ...
    def test_seed_job_uses_immutable_pass_instead_of_active_workspace(self):
        ...
        write_json(root / 'manifest.json', {'seed_policy': 'fresh-public-nllb', 'workspace': '/missing'})
        ...
        receipt = loop.seed_workspace(self.tools.workspace, root, '1.3b-distilled', 'passed')
        ...
        for changed in ({'returncode': 1, 'verdict': {'passed': True}},
            ...
            with self.assertRaises(ValueError):
                loop.seed_workspace(self.tools.workspace, root, '1.3b-distilled', 'passed')

    def test_same_model_seed_preserves_baseline_dependency_but_not_control_notes_or_passes(self):
        root = self.tools.run_dir / 'seed'
        source = root / 'workspace'
        source.mkdir(parents=True)
        write_json(root / 'manifest.json', {'seed_policy': 'fresh-public-nllb',
                   'model': '3.3b', 'workspace': str(source)})
        ...
        receipt = loop.seed_workspace(self.tools.workspace, root, '3.3b')
        for name in evidence:
    ...
    def test_cross_model_and_unknown_model_seed_still_drop_baseline_evidence(self):
        ...
        write_json(root / 'manifest.json', {'seed_policy': 'fresh-public-nllb', 'model': '600m'})
        for artifact_model in ('1.3b-distilled', None):
            frozen = {'files': files, 'code_sha256': loop.code_hash(files), 'model': artifact_model}
            write_json(root / 'submission.json', frozen)
            receipt = loop.seed_workspace(self.tools.workspace, root, '600m')
            self.assertEqual(receipt['retained_seed_notes'], ['REPORT.md', 'RESEARCH.md'])
            for name in ('BASELINE_BF16.md', 'BF16_BASELINE_X.md', 'CURRENT_BF16_SOURCE_1.md',
                         'CURRENT_BF16_IDENTITY.md', 'CANDIDATE_X.md', 'BFP8_X.md'):
                self.assertFalse((self.tools.workspace / name).exists())
                self.assertIn(name, receipt['omitted_seed_notes'])
    ...
    def test_seed_job_tampering_and_traversal_rejected(self):
        ...
        for job in ('pass', '../escape', '/absolute', '..'):
            with self.subTest(job=job), self.assertRaises(ValueError):
                loop.seed_workspace(self.tools.workspace, root, '3.3b', job)
    ...
    def archive(self, stage, result=None, precision='bf16', config=None, model='600m'):
    ...
    def test_submission_requires_passing_current_executable_source(self):
        self.delivery_receipt(job='one', model='600m')
    ...
    def test_submission_rejects_contradictory_missing_or_mismatched_receipts(self):
        folder, request, result = self.delivery_receipt(model='600m')
    ...
    def test_submission_latest_matching_receipt_cannot_fall_back_to_older_pass(self):
        first, _, _ = self.delivery_receipt(job='earlier', model='600m')
        later, request, result = self.delivery_receipt(job='later', model='600m')
    ...
    def test_submission_accepts_identified_passes_without_changing_precision_interface(self):
        for precision in loop.PRECISIONS:
            with self.subTest(precision=precision):
                self.delivery_receipt(model='1.3b-distilled', precision=precision)
                self.assertEqual(self.tools.submit(precision, '1.3b-distilled')['status'],
                                 'frozen_for_full_evaluation')
                submission = self.tools.run_dir / 'submission.json'
                saved = json.loads(submission.read_text())
                self.assertEqual((saved['model'], saved['precision']), ('1.3b-distilled', precision))
                submission.unlink()
    ...
    def test_cross_model_pass_never_deduplicates_or_authorizes_submission(self):
        self.archive('bringup', {'verdict': {'passed': True}}, model='600m')
        with self.assertRaises(ValueError):
            self.tools.submit(model='1.3b-distilled')
        with patch.object(loop.experiment, 'chia_remote', return_value='future') as remote:
            result = self.tools.evaluate('bringup', model='1.3b-distilled')
        self.assertNotEqual(result['job'], 'one')
        self.assertEqual(remote.call_args.args[2]['model'], '1.3b-distilled')
    ...
    def test_supervisor_facts_report_real_source_and_exact_exposed_controls(self):
        self.delivery_receipt(job='one', stage='smoke', model='600m')
        ...
        self.assertEqual(facts['workspace']['current_source_passes'][0]['model'], '600m')
    ...
    def delivery_receipt(self, job='delivery', model='1.3b-distilled', precision='bf16', stage='bringup'):
    ...
    def test_delivery_anchor_does_not_hide_other_models_or_precision_gaps(self):
        ...
        facts = loop.supervisor_facts(self.tools.workspace, self.tools.run_dir, self.config, '1.3b-distilled')
        self.assertEqual(facts['selected_model'], '1.3b-distilled')
        scope = facts['delivery_scope']
        self.assertEqual(scope['models'], ['600m', '1.3b-distilled', '3.3b'])
        ...
        for stage, job in [('smoke', 'anchor-smoke'), ('bringup', 'anchor-bringup')]:
            self.assertEqual(matrix['1.3b-distilled']['bf16'][stage]['status'], 'passed')
            self.assertEqual(matrix['1.3b-distilled']['bf16'][stage]['job'], job)
        for model in ('600m', '3.3b'):
            for precision in ('bf16', 'bfp8_b'):
                for cell in matrix[model][precision].values():
                    self.assertEqual(cell['status'], 'missing')
        self.assertEqual(matrix['1.3b-distilled']['bfp8_b']['bringup']['status'], 'missing')
        self.assertIn('not IEEE FP8', scope['precision_policy'])
        self.assertIn('ONE reusable', facts['objective'])
    ...
    def test_delivery_never_inherits_source_cluster_suite_or_precision_passes(self):
        ...
        cases = [({'code_sha256': 'old-source'}, {}, {}, 'historical'),
                 ...
                 ({}, {'model': '600m'}, {}, 'unverified'),
                 ({}, {'precision': 'bfp8_b'}, {}, 'unverified'),
                 ({}, {}, {'model_key': '600m'}, 'unverified'),
        ...
        for req, res, verdict, expected in cases:
            with self.subTest(req=req, res=res, verdict=verdict):
                ...
                self.assertEqual(matrix['1.3b-distilled']['bf16']['bringup']['status'], expected)
                self.assertEqual(matrix['1.3b-distilled']['bfp8_b']['bringup']['status'], 'missing')
    ...
    def test_delivery_latest_current_failure_or_pending_supersedes_earlier_pass(self):
        ...
        for status in ('failed', 'pending'):
            if status == 'failed':
                ...
                (later / 'result.json').unlink()
            facts = loop.supervisor_facts(self.tools.workspace, self.tools.run_dir, self.config)
            cell = facts['delivery_scope']['short_gates']['1.3b-distilled']['bf16']['bringup']
        ...
        cell = loop.supervisor_facts(self.tools.workspace, self.tools.run_dir,
            self.config)['delivery_scope']['short_gates']['1.3b-distilled']['bf16']['bringup']
    ...
    def test_delivery_bfp8_evidence_is_separate_and_reference_study_is_not_a_tt_gate(self):
        self.delivery_receipt(job='tt-bfp8', model='3.3b', precision='bfp8_b')
        folder, request, _ = self.delivery_receipt(job='cuda-study', model='600m')
        ...
        self.assertEqual(matrix['3.3b']['bfp8_b']['bringup']['status'], 'passed')
        self.assertEqual(matrix['3.3b']['bfp8_b']['bringup']['job'], 'tt-bfp8')
        self.assertEqual(matrix['3.3b']['bf16']['bringup']['status'], 'missing')
        self.assertEqual(matrix['600m']['bf16']['bringup']['status'], 'missing')

    def test_delivery_preserves_legacy_other_precision_pass_shape(self):
        self.archive('smoke', {'verdict': {'passed': True}}, precision='fp32')
        facts = loop.supervisor_facts(self.tools.workspace, self.tools.run_dir, self.config)
        self.assertEqual(facts['workspace']['current_source_passes'],
                         [{'stage': 'smoke', 'model': '600m', 'precision': 'fp32'}])
        self.assertEqual(facts['delivery_scope']['short_gates']['600m']['bf16']['smoke']['status'], 'missing')
    ...
    def test_diagnostic_notes_do_not_hide_implementation_from_supervisor(self):
        ...
        (self.tools.workspace / 'translate.py').write_text('# standalone entry point')
        facts = loop.supervisor_facts(self.tools.workspace, self.tools.run_dir, self.config)
        workspace = facts['workspace']
        self.assertIn('backend.py', workspace['file_names'])
        self.assertIn('translate.py', workspace['implementation_files'])
        ...
        self.assertIn('standalone text translation CLI', facts['objective'])
    ...
    def test_frozen_environment_uses_archive_and_checks_docs_and_registry(self):
        ...
        self.assertEqual(env['NLLB_REPOSITORY_ROOT'], '/workspace/original-repository')
    ...
    def test_campaign_default_model_reaches_run_and_evaluate(self):
        self.tools.model = '1.3b-distilled'
        with patch.object(loop.experiment, 'chia_remote', return_value='future') as remote:
            self.tools.run('python probe.py')
            self.assertEqual(remote.call_args.args[2]['model'], '1.3b-distilled')
            self.tools.evaluate()
            self.assertEqual(remote.call_args.args[2]['model'], '1.3b-distilled')
            self.tools.evaluate(model='600m')
            self.assertEqual(remote.call_args.args[2]['model'], '600m')
    ...
    def test_explicit_profile_model_selects_checkpoint_and_its_public_suite(self):
        ...
        with patch.object(loop.experiment, 'chia_remote', return_value='future') as remote:
            ...
            self.assertEqual(remote.call_args.args[2]['model'], '1.3b-distilled')
        self.assertEqual(self.tools.model, '1.3b-distilled')
    ...
    def test_profile_model_reaches_cached_inputs_and_remote_runner_without_assessment(self):
        request = {'kind': 'run', 'command': 'python profile.py',
                   'public_input_stage': 'smoke', 'model': '3.3b'}
        with patch.object(loop, 'cached_reference', return_value={'tt_dir': '/prepared/3.3b/smoke'}) as cached, \
        ...
        cached.assert_called_once_with(str(self.tools.run_dir), 'smoke', self.config, '3.3b')
        self.assertEqual(tt.call_args.args[2]['model'], '3.3b')
        self.assertEqual(tt.call_args.args[2]['public_input_stage'], 'smoke')
        self.assertEqual(result['model'], '3.3b')
    ...
    def test_supervisor_can_see_explicit_profile_models_and_anchor_default(self):
        facts = loop.supervisor_facts(self.tools.workspace, self.tools.run_dir, self.config, '1.3b-distilled')
        self.assertEqual(facts['controls']['run']['model'], ['600m', '1.3b-distilled', '3.3b'])
        self.assertEqual(facts['controls']['run']['default_model'], '1.3b-distilled')
    ...
    def test_seed_preserves_code_but_replaces_instructions_and_does_not_copy_passes(self):
        ...
        with self.assertRaises(ValueError):
            loop.seed_workspace(self.tools.workspace, root, '3.3b')

    def test_seed_submission_hash_is_verified(self):
        root = self.tools.run_dir / 'seed'
        write_json(root / 'manifest.json', {'seed_files': ['CONTRACT.md', 'TT_GUIDE.md', 'TASK.md']})
    ...
    def test_baselines_are_separate_per_model_precision_and_actual_suite(self):
        for model, precision in (('600m', 'bf16'), ('1.3b-distilled', 'bf16'), ('600m', 'fp32')):
            actual_suite = loop.digest({'model': model})
    ...
    def test_reference_paths_and_cli_are_model_specific(self):
        ...
        for model in ('600m', '1.3b-distilled'):
            with patch.object(loop, 'upload'), patch.object(loop, 'reference_job') as job, \
                 patch.object(loop, 'ssh', return_value=b'archive'):
                result = loop.reference._chia_original(str(run), 'smoke', config, model)
            self.assertIn('/' + model + '/smoke', result['reference_dir'])
            self.assertTrue(result['tt_dir'].endswith('/' + model + '/smoke'))
            self.assertIn('--model ' + model, job.call_args.args[2])

    def test_ready_deployed_reference_skips_gpu_resource_queue(self):
        run = self.tools.run_dir
        config = {'tt': {'work_dir': '/tt'}, 'reference': {'work_dir': '/gpu', 'python': '/env/python'}}
        key = loop.digest(config['reference'])[:16]
        cache = run / 'references' / key / '600m/smoke'
        expected = {'reference_dir': '/gpu/' + run.name + '/references/' + key + '/600m/smoke',
                    'tt_dir': '/tt/' + run.name + '/references/600m/smoke', 'model': '600m', 'stage': 'smoke',
        ...
        request = {'kind': 'evaluate', 'stage': 'smoke', 'model': '600m'}
        ...
        for altered in ({**expected, 'model': '3.3b'}, {**expected, 'stage': 'full'},
                        {**expected, 'endpoint_sha256': 'wrong'}):
            write_json(cache / 'ready.json', altered)
            self.assertIsNone(loop.cached_reference(run, 'smoke', config, '600m'))
        write_json(cache / 'ready.json', expected)
        ...
        with patch.object(loop.reference, 'chia_remote', return_value='future') as reference, \
            ...
            reference.assert_called_once_with(str(run), 'smoke', config, '600m')
    ...
    def test_supervisor_tracks_distinct_candidate_outcomes_and_baseline_evidence(self):
        ...
        facts = loop.supervisor_facts(self.tools.workspace, self.tools.run_dir, self.config)
        self.assertEqual(facts['evaluated_candidates'], [{'model': '600m', 'precision': 'bf16',
    ...
    def reference_fixture(self, stage='smoke'):
        import evaluate
        pinned = loop.model_spec('600m')
        corpus = {'sample': 'fixture'}
        with patch.object(evaluate, 'CORPUS_SHA256', loop.digest(corpus)):
            expected, _ = evaluate.make_suite(corpus, stage)
        spec = {k: v for k, v in expected.items() if k not in ('model_key', 'model_manifest_sha256')}
        spec.update(version=1, weight_sha256=loop.WEIGHT_SHA256)
        meta = {'parameter_dtypes': ['torch.float32'], 'weight_sha256': loop.WEIGHT_SHA256,
    ...
    def test_reference_archive_rejects_wrong_identity_precision_cases_and_unsafe_entries(self):
        ...
        with patch.object(evaluate, 'CORPUS_SHA256', loop.digest(corpus)):
            self.assertEqual(loop.validate_reference_archive(path, corpus, 'smoke', '600m'), good)
            with self.assertRaises(ValueError):
                loop.validate_reference_archive(path, corpus, 'smoke', '1.3b-distilled')
            for name, field, value in [('input/suite.json', 'model', 'wrong'),
                ...
                with self.subTest(field=field), self.assertRaises(ValueError):
                    loop.validate_reference_archive(path, corpus, 'smoke', '600m')
    ...
    def test_imported_reference_bytes_and_provenance_are_preserved_without_passes(self):
        ...
        for stage in ('smoke', 'bringup'):
            ...
            folder = source / 'references' / key / stage
        ...
        with patch.object(evaluate, 'CORPUS_SHA256', loop.digest(corpus)), \
            ...
            loop.import_references(run, source, config, '600m')
            self.assertEqual(ssh.call_count, 4)
            for stage in originals:
                folder = run / 'references' / key / '600m' / stage
                ...
                self.assertIsNotNone(loop.cached_reference(run, stage, config, '600m'))
            ...
            (source / 'references' / key / 'bringup/ready.json').unlink()
            ssh.reset_mock()
            with self.assertRaises(ValueError):
                loop.import_references(run, source, config, '600m')
            ssh.assert_not_called()
            write_json(source / 'references' / key / 'bringup/ready.json', {'endpoint_sha256': 'wrong'})
            with self.assertRaises(ValueError):
                loop.import_references(run, source, config, '600m')
            ssh.assert_not_called()
    ...
    def test_profile_run_requests_public_inputs_without_evaluation_dedup(self):
        ...
        self.assertEqual(saved['code_sha256'], loop.code_hash(loop.snapshot(self.tools.workspace)))
        self.assertEqual(saved['suite_definition_sha256'], loop.suite_identity(self.tools.run_dir, '600m', 'bringup'))
        with self.assertRaises(ValueError):
    ...
    def test_profile_run_resolves_cached_reference_without_assessment_or_gpu_queue(self):
        ...
        cached.assert_called_once_with(str(self.tools.run_dir), 'smoke', self.config, '600m')
        gpu.assert_not_called()
        ssh.assert_not_called()
        self.assertNotIn('verdict', result)
        self.assertEqual(tt.call_args.args[2]['public_input_stage'], 'smoke')
        ...
        gpu.assert_called_once_with(str(self.tools.run_dir), 'smoke', self.config, '600m')
