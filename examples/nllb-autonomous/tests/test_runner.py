...
class RunnerTests(unittest.TestCase):
    ...
    def config(self, root):
        return {'metal': '/deployment/tt-metal', 'python': '/deployment/python/bin/python',
                'weights': '/models/nllb/checkpoint.bin', 'health_dir': str(root / 'health'),
    ...
    def test_runtime_configuration_has_no_host_or_user_dependency(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(runner.WEIGHTS, Path('/models/nllb/checkpoint.bin'))
            index = args.index('/deployment/python')
            self.assertEqual(args[index - 1], '--ro-bind')
            self.assertNotIn('/models/nllb', args)
            self.assertIn('/models/nllb/checkpoint.bin', args)
    ...
    def test_sharded_mount_exposes_only_selected_manifest_files(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            checkpoint = root / 'weights3b'
            checkpoint.mkdir()
            spec = runner.model_spec('3.3b')
            ...
            runner.configure({**self.config(root), 'weights_by_model': {'3.3b': str(checkpoint)}})
            args = runner.sandbox(root, root / 'job', '0000:02:00.0', '7', '3.3b')
            self.assertEqual(runner.checkpoint('3.3b'), (checkpoint, '/weights'))
            self.assertNotIn(str(checkpoint), args)
            self.assertNotIn(str(checkpoint / 'earlier-private-backend.py'), args)
            self.assertNotIn('/models/nllb/checkpoint.bin', args)
            self.assertNotIn(str(cached.parent), args)
            for filename in spec['files']:
                self.assertIn(str((checkpoint / filename).resolve()), args)

    def test_model_mapping_rejects_unknown_missing_and_sharded_file_modes(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            with self.assertRaises(ValueError):
                runner.configure({**self.config(root), 'weights_by_model': {'1.3b': '/weights/checkpoint.bin'}})
            runner.configure(self.config(root))
            with self.assertRaises(ValueError):
                runner.checkpoint('1.3b-distilled')
            runner.configure({**self.config(root), 'weights_by_model': {'3.3b': '/weights/shard.bin'}})
            with self.assertRaises(ValueError):
                runner.checkpoint('3.3b')

    def test_evaluation_uses_model_specific_inputs_and_excludes_extra_files(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / 'harness').mkdir()
            config = {**self.config(root), 'weights_by_model': {'1.3b-distilled': '/models/large/pytorch_model.bin'}}
            (root / 'harness/runtime.json').write_text(json.dumps(config))
            inputs = root / 'references/1.3b-distilled/smoke/input'
            inputs.mkdir(parents=True)
            (inputs / 'suite.json').write_text(json.dumps({'model_key': '1.3b-distilled'}))
            ...
            with patch.object(runner, 'cards', return_value={'0000:02:00.0': '7'}), \
                ...
                for kind in ('evaluate', 'run'):
                    with self.subTest(kind=kind):
                        (inputs / 'suite.json').write_text(json.dumps({'model_key': '1.3b-distilled'}))
                        request = {'kind': kind, 'job': 'larger-' + kind, 'model': '1.3b-distilled'}
                        ...
                        self.assertEqual(result['model'], '1.3b-distilled')
                        self.assertIn('/models/large/pytorch_model.bin', captured[-1])
                        self.assertNotIn('/models/nllb/checkpoint.bin', captured[-1])
                        self.assertEqual({p.name for p in (root / 'jobs' / request['job'] / 'input').iterdir()},
                                         {'suite.json'})
                        (inputs / 'suite.json').write_text(json.dumps({'model_key': '600m'}))
                        with self.assertRaises(ValueError):
    ...
    def test_candidate_command_passes_validated_precision_and_weight_filename(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (root / 'references/smoke/input').mkdir(parents=True)
            captured = []
            def execute(args, job, *unused):
                captured.append(args)
            ...
            with patch.object(runner, 'cards', return_value={'0000:02:00.0': '7'}), \
                ...
                for precision in (None, 'fp32', 'fp16', 'bfp8_b'):
                    ...
                    self.assertEqual(captured[-1][-4:], ['--weights', '/weights/checkpoint.bin',
    ...
    def test_nondefault_model_preflight_and_hashing_timeout_do_not_require_600m(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / 'harness').mkdir()
            config = self.config(root)
            del config['weights']
            config['weights_by_model'] = {'1.3b-distilled': '/models/large/pytorch_model.bin'}
            (root / 'harness/runtime.json').write_text(json.dumps(config))
            (root / 'references/1.3b-distilled/smoke/input').mkdir(parents=True)
            current = {'returncode': 0, 'timeout': False}
            def execute(args, job, *unused):
                self.assertIn('/models/large/pytorch_model.bin', args)
                for filename in ('stdout.log', 'stderr.log'):
                    (job / filename).write_text('')
                return dict(current)
            with patch.object(runner, 'cards', return_value={'0000:02:00.0': '7'}), \
                ...
                result = runner.main(root, {'kind': 'preflight', 'pci': '0000:02:00.0', 'job': 'preflight', 'model': '1.3b-distilled'})
                ...
                result = runner.main(root, {'kind': 'evaluate', 'stage': 'smoke', 'job': 'slow-hash', 'model': '1.3b-distilled'})
    ...
    def test_full_deadline_changes_only_full_evaluation_and_ignores_candidate_override(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for stage in ('smoke', 'bringup', 'full'):
                (root / 'references' / stage / 'input').mkdir(parents=True)
            captured = []
    ...
    def test_profile_run_mounts_only_readonly_public_inputs_with_normal_timeout(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (root / 'harness').mkdir()
            (root / 'harness/runtime.json').write_text(json.dumps(self.config(root)))
            public = root / 'references/600m/smoke/input'
            public.mkdir(parents=True)
            for name, content in (('suite.json', '{"model_key":"600m","stage":"smoke"}'),
