...
def configure(config):
    ...
    if config.get('weights'):
        weights.setdefault('600m', config['weights'])
    if not weights:
    ...
    WEIGHTS_BY_MODEL, WEIGHTS = weights, weights.get('600m')
...
def sandbox(root, job, pci, node, model='600m'):
...
def main(root, request):
    ...
    if input_stage:
        ...
        if not reference.exists() and model == '600m':
            reference = root / 'references' / input_stage / 'input'
        if (reference / 'suite.json').exists():
            prepared = json.loads((reference / 'suite.json').read_text())
            if prepared.get('model_key', '600m') != model:
