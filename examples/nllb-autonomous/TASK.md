...
1. Read TT_GUIDE.md; inspect public implementations and pinned installed source.
...
2. Write backend.py and useful component checks. Use `nllb_run` for isolated
...
3. Run `nllb_evaluate(stage="smoke")` for one short real-model request. An
   ...
   Use `nllb_status(compact=True)` to collect asynchronous results; retain job IDs. Inspect
...
7. Establish repeated-request memory behavior with bounded diagnostics. If a
   ...
   using `nllb_log`. Report unresolved failures and unsupported behavior explicitly.
8. Call `nllb_submit` when the current source passes bringup and is ready. This
   freezes it for the independent full corpus. Full acceptance is not guaranteed.

Only `nllb_write` persists edits; files written by a remote shell are job-local.
Native Codex filesystem access is read-only; the separately authorized
`nllb_write` service enforces the allowed source paths. Use it for permitted
edits and STATE/REPORT, retaining any actual denial. CPU-only checks use bounded
`nllb_run` jobs on the TT host without opening a TT device; no separate local
...
`nllb_files/read/write/run/evaluate/status/log/submit` are your workflow tools.
`nllb_run(..., model="600m")` can select a registered checkpoint for standalone
...
healthy cards. Read truncated stdout/stderr with `nllb_log(job_id, offset_bytes=0)`
