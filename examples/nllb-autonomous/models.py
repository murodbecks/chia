"""Pinned NLLB checkpoint identities; verify bytes before loading model weights."""
import argparse
import hashlib
import json
...
def model_spec(name, registry=REGISTRY):
    ...
    expected = "pytorch_bin_sharded" if index else "pytorch_bin"
    if spec["checkpoint_format"] != expected or (not index and len(weights) != 1):
...
def verify_checkpoint(directory, name, *, registry=REGISTRY, all_files=False):
    """Hash without deserializing .bin files; accept normal HF cache symlinks.

    Default verifies config, every weight shard and the shard index. all_files
    also checks the recorded tokenizer/generation files. A passed receipt proves
