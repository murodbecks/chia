...
def suite_model(spec):
    name = spec.get("model_key", "600m")
    identity = model_identity(name)
    if spec.get("version") == 2:
        if spec.get("version") != 2 or any(spec.get(k) != v for k, v in identity.items()):
    ...
    elif spec.get("version") != 1 or name != "600m" or any(k in spec and spec[k] != identity[k] for k in ("model", "revision")):
...
def validate_config(config, model):
    if any(config.get(k) != v for k, v in model["architecture"].items()):
        raise ValueError("configuration does not match pinned model architecture")
...
def validate_receipt(receipt, identity, model, spec=None):
    ...
    if (not isinstance(receipt, dict) or receipt.get("model") != identity["model_key"] or receipt.get("repo_id") != identity["model"]
...
def validate_artifact(artifact, spec, identity):
    if spec.get("version", 1) >= 2 and (not isinstance(artifact, dict) or any(artifact.get(k) != v for k, v in identity.items())
...
def verify_weights(path, name):
    """Legacy single-file checkpoints and official sharded directories, never pickle inspection."""
...
def reference_execution():
    mode = os.getenv("NLLB_REFERENCE_EXECUTION", "slurm")
    if mode not in ("slurm", "direct") or socket.gethostname().lower().startswith(("login", "lo-")):
        raise RuntimeError("reference requires an explicitly configured GPU compute host")
...
def require_tt_precision(mode, runtime):
    ...
    dtype = {"bf16": "bfloat16", "fp32": "float32", "fp16": "float16", "bfp8_b": "bfloat8_b"}[mode]
...
def validate_precision(mode, policy, runtime=None):
    """Validate declarations, not a proof of every intermediate tensor's dtype."""
    require_tt_precision(mode, runtime)
    if not isinstance(policy, dict) or set(policy) != {"mode", "weights", "activations", "accumulation", "exceptions"}:
    ...
    require_tt_precision(policy["activations"], runtime)
...
def validate_input_preservation(expected, observed):
    ...
    for name, original in expected.items():
        ...
        if (not isinstance(value, np.ndarray) or value.dtype != original.dtype
                or not np.array_equal(value, original)):
