...
class ModelTests(unittest.TestCase):
    ...
    def test_real_registry_pins_chronos2_safetensors(self):
        spec = models.model_spec("chronos2")
        self.assertEqual(spec["repo_id"], "amazon/chronos-2")
        ...
        self.assertEqual(spec["architecture"]["d_model"], 768)
        self.assertEqual(spec["architecture"]["num_layers"], 12)
        self.assertEqual(len(spec["architecture"]["chronos_config"]["quantiles"]), 21)
        self.assertEqual(spec["architecture"]["chronos_config"]["input_patch_size"], 16)
        self.assertEqual(len(spec["files"]["model.safetensors"]["sha256"]), 64)
        with self.assertRaisesRegex(ValueError, "Unknown model"):
            models.model_spec("chronos")
    ...
    def test_nested_chronos_config_must_match(self):
        self.write("config.json", b'{"d_model":4,"chronos_config":{"input_patch_size":32}}')
        self.spec["architecture"] = {"d_model": 4, "chronos_config": {"input_patch_size": 32}}
        self.verify()
        self.spec["architecture"]["chronos_config"]["input_patch_size"] = 16
        with self.assertRaisesRegex(ValueError, "chronos_config"):
            self.verify()
