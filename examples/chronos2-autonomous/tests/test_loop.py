...
class LoopTests(unittest.TestCase):
    ...
    def test_status_exact_old_job_keeps_complete_gate_and_is_read_only(self):
        ...
        request = {'kind': 'evaluate', 'stage': 'bringup', 'model': 'chronos2',
    ...
    def test_status_exact_pending_is_local_and_preserves_request(self):
        request = {'kind': 'reference-study', 'model': 'chronos2', 'cluster_sha256': 'missing'}
    ...
    def test_status_exact_historical_dotted_reference_id(self):
        job_id = 'gpu-precision.chronos2-20260921T1020'
        request = {'kind': 'reference-study', 'model': 'chronos2'}
    ...
    def test_timeout_recovery_requires_real_probe_and_cannot_repeat(self):
        ...
        with patch.object(loop, 'remote') as remote:
            ...
            self.assertEqual(loop.recover_timed_out_card(
                self.tools.run_dir, self.config, health, 'chronos2'), health)
    ...
    def test_timeout_recovery_preserves_failed_or_ambiguous_probe(self):
        ...
        with patch.object(loop, 'remote', side_effect=TimeoutError('transport ambiguous')):
            with self.assertRaises(TimeoutError):
                loop.recover_timed_out_card(self.tools.run_dir, self.config, health, 'chronos2')
        ...
        with patch.object(loop, 'remote', side_effect=[{'returncode': 124, 'timeout': True}, health]):
            self.assertEqual(loop.recover_timed_out_card(
                self.tools.run_dir, replacement, health, 'chronos2'), health)
    ...
    def test_compact_status_preserves_failure_quality_identity_and_legacy_response(self):
        ...
        request = {'kind': 'evaluate', 'stage': 'smoke', 'model': 'chronos2', 'precision': 'bf16',
        ...
        result = {'returncode': 125, 'failure': 'host_memory_limit', 'timeout': False,
                  ...
                  'model': 'chronos2', 'precision': 'bf16', 'pci': '0000:41:00.0',
        ...
        self.assertEqual(compact['details']['tool'], 'chronos_log')
    ...
    def test_compact_status_keeps_old_pending_and_reference_study_details(self):
        ...
        write_json(run / 'jobs/study/request.json', {'kind': 'reference-study', 'model': 'chronos2'})
        ...
        for job in ('study', 'pending-reference'):
            self.assertEqual(compact[job]['details']['tool'], 'chronos_status')
    ...
    def test_log_registered_and_uses_job_archived_endpoint(self):
        ...
        self.assertIn('chronos_log', names)
    ...
    def test_run_punctuation_cannot_create_nested_mcp_config_keys(self):
        ...
        name = loop.tool_name(self.tools.run_dir / 'model-chronos2 with spaces')
        self.assertRegex(name, r'^chronos_[a-f0-9]{16}$')
        ...
        agent.agent_name, agent.config = 'chronos-worker', None
    ...
    def make_previous_run(self, name='previous', **manifest):
        ...
        write_json(root / 'manifest.json', {'seed_policy': 'fresh-public-chronos2', 'model': 'chronos2',
    ...
    def test_resume_carries_full_workspace_and_state_without_passes(self):
        ...
        receipt = loop.resume_workspace(self.tools.workspace, root, 'chronos2',
        ...
        with self.assertRaisesRegex(ValueError, 'needs an independent bringup pass'):
            self.tools.submit(model='chronos2')

    def test_resume_rejects_other_model_submitted_or_non_fresh_sources(self):
        root = self.make_previous_run(model='other')
        with self.assertRaisesRegex(ValueError, 'different model'):
            loop.resume_workspace(self.tools.workspace, root, 'chronos2', {'worker': 'claude', 'supervisor': 'claude'})
    ...
    def test_agent_environment_defaults_older_manifests_to_opencode(self):
        self.assertEqual(loop.agent_environment({}), {'CHRONOS_WORKER_BACKEND': 'opencode',
                                                      'CHRONOS_SUPERVISOR_BACKEND': 'opencode'})
        ...
        self.assertEqual(loop.agent_environment(manifest), {
            'CHRONOS_WORKER_BACKEND': 'claude', 'CHRONOS_WORKER_MODEL': 'claude-opus-5-5',
            'CHRONOS_SUPERVISOR_BACKEND': 'opencode', 'CHRONOS_SUPERVISOR_MODEL': 'zai-coding-plan/glm-5.3'})

    def test_complete_references_requires_both_stages(self):
        root = self.make_previous_run()
        stage = root / 'references/key/chronos2'
        for name in ('smoke/ready.json', 'smoke/reference.tar', 'bringup/ready.json'):
            (stage / name).parent.mkdir(parents=True, exist_ok=True)
            (stage / name).write_text('x')
        self.assertFalse(loop.complete_references(root, 'chronos2'))
        (stage / 'bringup/reference.tar').write_text('x')
        self.assertTrue(loop.complete_references(root, 'chronos2'))

    def test_seed_job_uses_immutable_pass_instead_of_active_workspace(self):
        root = self.tools.run_dir / 'seed'
        write_json(root / 'manifest.json', {'seed_policy': 'fresh-public-chronos2', 'workspace': '/missing'})
        ...
        receipt = loop.seed_workspace(self.tools.workspace, root, 'chronos2', 'passed')
        ...
        for changed in ({'returncode': 1, 'verdict': {'passed': True}},
            ...
            with self.assertRaises(ValueError):
                loop.seed_workspace(self.tools.workspace, root, 'chronos2', 'passed')

    def test_same_model_seed_preserves_baseline_dependency_but_not_control_notes_or_passes(self):
        root = self.tools.run_dir / 'seed'
        source = root / 'workspace'
        source.mkdir(parents=True)
        write_json(root / 'manifest.json', {'seed_policy': 'fresh-public-chronos2',
                   'model': 'chronos2', 'workspace': str(source)})
        ...
        write_json(root / 'jobs/old/result.json', {'verdict': {'passed': True}})
        receipt = loop.seed_workspace(self.tools.workspace, root, 'chronos2')
        for name in evidence:
    ...
    def test_cross_model_and_unknown_model_seed_still_drop_baseline_evidence(self):
        ...
        for artifact_model, expected in (('chronos2', sorted(list(evidence) + ['RESEARCH.md', 'REPORT.md'])),
            ...
            for name in evidence:
                self.assertEqual((self.tools.workspace / name).exists(), artifact_model == 'chronos2')
    ...
    def test_seed_job_tampering_and_traversal_rejected(self):
        ...
        for job in ('pass', '../escape', '/absolute', '..'):
            with self.subTest(job=job), self.assertRaises(ValueError):
                loop.seed_workspace(self.tools.workspace, root, 'chronos2', job)
    ...
    def archive(self, stage, result=None, precision='bf16', config=None, model='chronos2'):
    ...
    def test_submission_requires_passing_current_executable_source(self):
        self.delivery_receipt(job='one', model='chronos2')
    ...
    def test_submission_rejects_contradictory_missing_or_mismatched_receipts(self):
        folder, request, result = self.delivery_receipt(model='chronos2')
    ...
    def test_submission_latest_matching_receipt_cannot_fall_back_to_older_pass(self):
        first, _, _ = self.delivery_receipt(job='earlier', model='chronos2')
        later, request, result = self.delivery_receipt(job='later', model='chronos2')
    ...
    def test_submission_accepts_identified_passes_without_changing_precision_interface(self):
        for precision in loop.PRECISIONS:
            with self.subTest(precision=precision):
                self.delivery_receipt(model='chronos2', precision=precision)
                self.assertEqual(self.tools.submit(precision, 'chronos2')['status'],
                                 'frozen_for_full_evaluation')
                submission = self.tools.run_dir / 'submission.json'
                saved = json.loads(submission.read_text())
                self.assertEqual((saved['model'], saved['precision']), ('chronos2', precision))
                submission.unlink()
    ...
    def test_supervisor_facts_report_real_source_and_exact_exposed_controls(self):
        self.delivery_receipt(job='one', stage='smoke', model='chronos2')
        ...
        self.assertEqual(facts['workspace']['current_source_passes'][0]['model'], 'chronos2')
    ...
    def delivery_receipt(self, job='delivery', model='chronos2', precision='bf16', stage='bringup'):
    ...
    def test_delivery_anchor_does_not_hide_other_models_or_precision_gaps(self):
        ...
        facts = loop.supervisor_facts(self.tools.workspace, self.tools.run_dir, self.config, 'chronos2')
        self.assertEqual(facts['selected_model'], 'chronos2')
        scope = facts['delivery_scope']
        self.assertEqual(scope['models'], ['chronos2'])
        ...
        for stage, job in [('smoke', 'anchor-smoke'), ('bringup', 'anchor-bringup')]:
            self.assertEqual(matrix['chronos2']['bf16'][stage]['status'], 'passed')
            self.assertEqual(matrix['chronos2']['bf16'][stage]['job'], job)
        for stage in ('smoke', 'bringup'):
            self.assertEqual(matrix['chronos2']['bfp8_b'][stage]['status'], 'missing')
            self.assertEqual(matrix['chronos2']['bfp8_b'][stage]['job'], None)
    ...
    def test_delivery_never_inherits_source_cluster_suite_or_precision_passes(self):
        ...
        for req, res, verdict, expected in cases:
            with self.subTest(req=req, res=res, verdict=verdict):
                ...
                self.assertEqual(matrix['chronos2']['bf16']['bringup']['status'], expected)
                self.assertEqual(matrix['chronos2']['bfp8_b']['bringup']['status'], 'missing')
    ...
    def test_delivery_latest_current_failure_or_pending_supersedes_earlier_pass(self):
        ...
        for status in ('failed', 'pending'):
            if status == 'failed':
                ...
                (later / 'result.json').unlink()
            facts = loop.supervisor_facts(self.tools.workspace, self.tools.run_dir, self.config)
            cell = facts['delivery_scope']['short_gates']['chronos2']['bf16']['bringup']
        ...
        cell = loop.supervisor_facts(self.tools.workspace, self.tools.run_dir,
            self.config)['delivery_scope']['short_gates']['chronos2']['bf16']['bringup']
    ...
    def test_delivery_bfp8_evidence_is_separate_and_reference_study_is_not_a_tt_gate(self):
        self.delivery_receipt(job='tt-bfp8', model='chronos2', precision='bfp8_b')
        folder, request, _ = self.delivery_receipt(job='cuda-study', model='chronos2')
        ...
        self.assertEqual(matrix['chronos2']['bfp8_b']['bringup']['status'], 'passed')
        self.assertEqual(matrix['chronos2']['bfp8_b']['bringup']['job'], 'tt-bfp8')
        self.assertEqual(matrix['chronos2']['bf16']['bringup']['status'], 'missing')
        self.assertEqual(matrix['chronos2']['bf16']['bringup']['status'], 'missing')
    ...
    def test_delivery_preserves_legacy_other_precision_pass_shape(self):
        ...
        facts = loop.supervisor_facts(self.tools.workspace, self.tools.run_dir, self.config)
        self.assertEqual(facts['workspace']['current_source_passes'],
                         [{'stage': 'smoke', 'model': 'chronos2', 'precision': 'fp32'}])
        self.assertEqual(facts['delivery_scope']['short_gates']['chronos2']['bf16']['smoke']['status'], 'missing')
    ...
    def test_frozen_environment_uses_archive_and_checks_docs_and_registry(self):
        ...
        self.assertEqual(env['CHRONOS_REPOSITORY_ROOT'], '/workspace/original-repository')
    ...
    def test_campaign_default_model_reaches_run_and_evaluate(self):
        self.tools.model = 'chronos2'
        with patch.object(loop.experiment, 'chia_remote', return_value='future') as remote:
            self.tools.run('python probe.py')
            self.assertEqual(remote.call_args.args[2]['model'], 'chronos2')
            self.tools.evaluate()
            self.assertEqual(remote.call_args.args[2]['model'], 'chronos2')
            self.tools.evaluate(model='chronos2')
            self.assertEqual(remote.call_args.args[2]['model'], 'chronos2')

    def test_explicit_profile_model_selects_checkpoint_and_its_public_suite(self):
        self.tools.model = 'chronos2'
        with patch.object(loop.experiment, 'chia_remote', return_value='future') as remote:
            for model in ('chronos2', 'chronos2'):
            ...
            self.assertEqual(remote.call_args.args[2]['model'], 'chronos2')
        self.assertEqual(self.tools.model, 'chronos2')
    ...
    def test_profile_model_reaches_cached_inputs_and_remote_runner_without_assessment(self):
        request = {'kind': 'run', 'command': 'python profile.py',
                   'public_input_stage': 'smoke', 'model': 'chronos2'}
        with patch.object(loop, 'cached_reference', return_value={'tt_dir': '/prepared/chronos2/smoke'}) as cached, \
        ...
        cached.assert_called_once_with(str(self.tools.run_dir), 'smoke', self.config, 'chronos2')
        self.assertEqual(tt.call_args.args[2]['model'], 'chronos2')
        self.assertEqual(tt.call_args.args[2]['public_input_stage'], 'smoke')
        self.assertEqual(result['model'], 'chronos2')
    ...
    def test_supervisor_can_see_explicit_profile_models_and_anchor_default(self):
        facts = loop.supervisor_facts(self.tools.workspace, self.tools.run_dir, self.config, 'chronos2')
        self.assertEqual(facts['controls']['run']['model'], ['chronos2'])
        self.assertEqual(facts['controls']['run']['default_model'], 'chronos2')
    ...
    def test_seed_preserves_code_but_replaces_instructions_and_does_not_copy_passes(self):
        ...
        write_json(root / 'manifest.json', {'workspace': str(workspace),
                   'seed_files': ['CONTRACT.md', 'TT_GUIDE.md', 'TASK.md', 'BENCHMARK.md', 'LESSONS_NLLB.md']})
        write_json(root / 'jobs/123/result.json', {'verdict': {'passed': True}})
        (self.tools.workspace / 'TASK.md').write_text('trusted new target')
        provenance = loop.seed_workspace(self.tools.workspace, root, 'chronos2')
        ...
        with self.assertRaises(ValueError):
            loop.seed_workspace(self.tools.workspace, root, 'chronos2')

    def test_seed_submission_hash_is_verified(self):
        root = self.tools.run_dir / 'seed'
        write_json(root / 'manifest.json', {'seed_files': ['CONTRACT.md', 'TT_GUIDE.md', 'TASK.md', 'BENCHMARK.md', 'LESSONS_NLLB.md']})
    ...
    def test_baselines_are_separate_per_model_precision_and_actual_suite(self):
        for model, precision in (('chronos2', 'bf16'), ('chronos2', 'fp32'), ('chronos2', 'bfp8_b')):
    ...
    def test_reference_paths_and_cli_are_model_specific(self):
        ...
        for stage in ('smoke', 'bringup'):
            with patch.object(loop, 'upload'), patch.object(loop, 'reference_job') as job, \
                ...
                result = loop.reference._chia_original(str(run), stage, config, 'chronos2')
            self.assertIn('/chronos2/' + stage, result['reference_dir'])
            self.assertTrue(result['tt_dir'].endswith('/chronos2/' + stage))
            self.assertIn('--model chronos2', job.call_args.args[2])

    def test_ready_deployed_reference_skips_gpu_resource_queue(self):
        run = self.tools.run_dir
        config = {'tt': {'work_dir': '/tt'}, 'reference': {'work_dir': '/gpu', 'python': '/env/python'}}
        key = loop.digest(config['reference'])[:16]
        cache = run / 'references' / key / 'chronos2/smoke'
        expected = {'reference_dir': '/gpu/' + run.name + '/references/' + key + '/chronos2/smoke',
                    'tt_dir': '/tt/' + run.name + '/references/chronos2/smoke', 'model': 'chronos2', 'stage': 'smoke',
        ...
        request = {'kind': 'evaluate', 'stage': 'smoke', 'model': 'chronos2'}
        ...
        for altered in ({**expected, 'model': 'other-model'}, {**expected, 'stage': 'full'},
            ...
            self.assertIsNone(loop.cached_reference(run, 'smoke', config, 'chronos2'))
        write_json(cache / 'ready.json', expected)
        ...
        with patch.object(loop.reference, 'chia_remote', return_value='future') as reference, \
            ...
            reference.assert_called_once_with(str(run), 'smoke', config, 'chronos2')
    ...
    def test_supervisor_tracks_distinct_candidate_outcomes_and_baseline_evidence(self):
        ...
        facts = loop.supervisor_facts(self.tools.workspace, self.tools.run_dir, self.config)
        self.assertEqual(facts['evaluated_candidates'], [{'model': 'chronos2', 'precision': 'bf16',
    ...
    def reference_fixture(self, stage='smoke'):
        import evaluate
        pinned = loop.model_spec('chronos2')
        corpus = {'schema_version': 1, 'task': 'fixture', 'protocol': 'fixture', 'sources': [],
                  'series': {key: [float(j % 97) / 7.0 for j in range(1536)] for key in (
        ...
        receipt = {'model': 'chronos2', 'repo_id': pinned['repo_id'], 'revision': pinned['revision'],
    ...
    def test_reference_archive_rejects_wrong_identity_precision_cases_and_unsafe_entries(self):
        ...
        with patch.object(evaluate, 'CORPUS_SHA256', loop.digest(corpus)):
            self.assertEqual(loop.validate_reference_archive(path, corpus, 'smoke', 'chronos2'), good)
            with self.assertRaises(ValueError):
            ...
            for name, field, value in [('input/suite.json', 'model', 'wrong'),
                ...
                with self.subTest(field=field), self.assertRaises(ValueError):
                    loop.validate_reference_archive(path, corpus, 'smoke', 'chronos2')
    ...
    def test_imported_reference_bytes_and_provenance_are_preserved_without_passes(self):
        ...
        for stage in ('smoke', 'bringup'):
            ...
            folder = source / 'references' / key / 'chronos2' / stage
        ...
        with patch.object(evaluate, 'CORPUS_SHA256', loop.digest(corpus)), \
            ...
            loop.import_references(run, source, config, 'chronos2')
            self.assertEqual(ssh.call_count, 4)
            for stage in originals:
                folder = run / 'references' / key / 'chronos2' / stage
                ...
                self.assertIsNotNone(loop.cached_reference(run, stage, config, 'chronos2'))
            ...
            (source / 'references' / key / 'chronos2' / 'bringup/ready.json').unlink()
            ssh.reset_mock()
            with self.assertRaises(ValueError):
                loop.import_references(run, source, config, 'chronos2')
            ssh.assert_not_called()
            write_json(source / 'references' / key / 'chronos2' / 'bringup/ready.json', {'endpoint_sha256': 'wrong'})
            with self.assertRaises(ValueError):
                loop.import_references(run, source, config, 'chronos2')
            ssh.assert_not_called()
    ...
    def test_profile_run_requests_public_inputs_without_evaluation_dedup(self):
        ...
        self.assertEqual(saved['code_sha256'], loop.code_hash(loop.snapshot(self.tools.workspace)))
        self.assertEqual(saved['suite_definition_sha256'], loop.suite_identity(self.tools.run_dir, 'chronos2', 'bringup'))
        with self.assertRaises(ValueError):
    ...
    def test_profile_run_resolves_cached_reference_without_assessment_or_gpu_queue(self):
        ...
        gpu.assert_called_once_with(str(self.tools.run_dir), 'smoke', self.config, 'chronos2')
