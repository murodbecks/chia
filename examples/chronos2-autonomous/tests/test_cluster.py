...
class ClusterTests(unittest.TestCase):
    ...
    def test_model_checkpoint_inventory_is_explicit_and_validated(self):
        ...
        raw['porting']['tt']['weights_by_model'] = {'chronos2': '/models/chronos2'}
        self.assertEqual(self.load(raw)['tt']['weights_by_model']['chronos2'], '/models/chronos2')
        for value in ({'unknown-model': '/models/ambiguous'}, {'chronos2': '../relative'},
                      {'chronos2': '/models/../other'}, {'chronos2': 1}, []):
    ...
    def test_direct_and_slurm_launch_once_then_reuse_record(self):
        for executor in ('direct', 'slurm'):
            ...
            with patch.object(cluster, 'upload') as upload, patch.object(cluster, 'ssh', side_effect=[b'12345\n', b'0\n']) as ssh:
                ...
                self.assertIn('CHRONOS_REFERENCE_EXECUTION=' + executor, script)
