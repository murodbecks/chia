...
Collect genuine results with `chronos_status(compact=True)`, which includes the
structured verdict and request/source/cluster identity. `chronos_log` retrieves
stdout/stderr only. The isolated `chronos_run` environment receives public
...
(config.json + model.safetensors). Inspect these with a bounded `chronos_run`
