...
set -euo pipefail
reference_root="${1:-$HOME/chronos2-autonomous}"
...
"$reference_python" -m pip install --index-url https://pypi.org/simple "chronos-forecasting>=2.0,<3" numpy==2.4.4 --report "$reference_root/reference-install.json"
"$reference_python" -m pip check
"$reference_python" -m pip freeze > "$reference_root/requirements.lock.txt"
"$reference_python" - <<'PY'
import torch
from chronos import Chronos2Pipeline  # noqa: F401  (import guard)
