...
def configure(config):
    ...
    if config.get('weights'):
        weights.setdefault('chronos2', config['weights'])
    if not weights:
    ...
    WEIGHTS_BY_MODEL, WEIGHTS = weights, weights.get('chronos2')
...
def sandbox(root, job, pci, node, model='chronos2'):
...
def main(root, request):
    ...
    if input_stage:
        ...
        if (reference / 'suite.json').exists():
            ...
            if prepared.get('model_key', 'chronos2') != model:
