...
Continue an earlier campaign in a new archived run, optionally on another agent CLI:
     python loop.py --corpus /path/to/corpus.json --hours 4 --detach \
         --resume runs/<earlier-run> --agent claude
Agents run on the local opencode CLI (z.ai GLM coding plan, default) or the local
Claude Code CLI (--agent claude); --supervisor-agent selects the supervisor separately.
Neither agent receives an earlier private port.
"""
import argparse
...
def frozen_environment(run, manifest):
    ...
    return {**os.environ, 'PYTHONPATH': str(frozen), 'CHRONOS_REPOSITORY_ROOT': str(repository)}
...
def agent_environment(manifest):
    ...
    for role in ('worker', 'supervisor'):
        environment['CHRONOS_' + role.upper() + '_BACKEND'] = backends.get(role, 'opencode')
        if models.get(role):
            environment['CHRONOS_' + role.upper() + '_MODEL'] = models[role]
    return environment
...
if __name__ == '__main__' and '--controller' in sys.argv:
    ...
    os.environ.update({k: environment[k] for k in ('PYTHONPATH', 'CHRONOS_REPOSITORY_ROOT')})
...
WEIGHT_SHA256 = model_spec("chronos2")["files"]["model.safetensors"]["sha256"]
from cluster import activated, identity as digest, load as load_cluster, ssh, upload, remote, reference_job

HERE = Path(__file__).resolve().parent
CORPUS_HASH = 'f0b08df4fe80385da1d3e9a05e3779708c4e808b2da581aa974fea9505d64ae3'
...
def tool_name(run):
    ...
    return 'chronos_' + hashlib.sha256(str(Path(run).resolve()).encode()).hexdigest()[:16]
...
def supervisor_facts(workspace, run, config, model="chronos2"):
    ...
    for path in sorted((Path(run) / 'jobs').glob('*/request.json'), key=lambda p: p.stat().st_mtime):
        ...
        item = {'id': path.parent.name, 'kind': request.get('kind', 'evaluate'),
                'stage': request.get('stage', 'experiment'), 'model': request.get('model', 'chronos2'),
    ...
    return {'objective': 'Deliver ONE production-level reusable Chronos-2 TTNN source packaged in '
                         'tt-metal style (tt/, demo/, reference/, tests/, benchmarks/, docs/ with SPDX '
                         'headers) suitable for an upstream tt-metal PR. Establish measured TT-side '
                         'PyTorch/ONNX baselines on the fixed suites FIRST and preserve them; pass smoke, '
                         'then bringup against the independent FP32 oracle; profile and optimize against '
                         'the recorded baseline with matched same-precision comparisons; assemble the '
                         'tt-metal-style package and portable tests; then freeze for full validation. '
                         'A first bringup pass alone is not the goal, and optimization claims without a '
                         'recorded baseline are invalid. Advance independent implementation when runtime '
                         'diagnosis blocks final evaluation. Follow LESSONS_NLLB.md. ',
            ...
            'delivery_scope': {'models': list(delivery_models), 'development_anchor': model,
                ...
                'precision_policy': 'BF16 primary; investigate supported BFP8_B with honest '
            ...
            'controls': {'run': {'public_input_stage': ['', 'smoke', 'bringup'],
                         ...
                         'evaluate': {'stage': ['smoke', 'bringup'], 'precision': list(PRECISIONS),
                                      'model': ['chronos2']},
                         'fixed_suites': {'smoke': {'cases': 1, 'context': 512, 'horizon': 64},
                                          'bringup': {'cases': 8, 'context': 512, 'horizon': 64}},
...
def seed_workspace(workspace, seed_run, model, seed_job=None):
    ...
    trusted = ('CONTRACT.md', 'TT_GUIDE.md', 'TASK.md', 'BENCHMARK.md', 'LESSONS_NLLB.md')
    if (sorted(manifest.get('seed_files', [])) != sorted(trusted)
            and manifest.get('seed_policy') != 'fresh-public-chronos2'):
        raise ValueError('seed must be a fresh public-source Chronos-2 campaign')
...
def resume_workspace(workspace, previous_run, model, backends, note=''):
    ...
    manifest = json.loads((root / 'manifest.json').read_text())
    if manifest.get('seed_policy') != 'fresh-public-chronos2':
        raise ValueError('resume source must be a fresh public-source Chronos-2 campaign')
    ...
    header = ('# Resumed campaign\n\n'
        ...
        'chronos_status(job_id=...) and chronos_log still read them as historical receipts, but no '
...
def prepare(run, config):
    ...
    weights = endpoint.get('weights_by_model', {'chronos2': endpoint.get('weights')})
...
def validate_reference_archive(path, corpus, stage, model):
    ...
    with tarfile.open(fileobj=io.BytesIO(data)) as archive:
        ...
        for key in ('stage', 'corpus_sha256', 'max_context', 'gates', 'timed_repeats'):
...
def reference(run_dir, stage, config, model="chronos2"):
    model_spec(model)
...
def experiment(run_dir, job, request, config):
    ...
    try:
        model = request.get('model', 'chronos2')
        ...
        result['model'] = request.get('model', 'chronos2')
...
@ChiaFunction(resources={'reference': 1}, max_retries=0)
def precision_study(run_dir, job, config, model="chronos2"):
...
class PortTools(ChiaTool):
    ...
    def write(self, path: str, content: str) -> dict:
        if Path(path).as_posix() in ('CONTRACT.md', 'TT_GUIDE.md', 'TASK.md', 'BENCHMARK.md', 'LESSONS_NLLB.md') or (self.run_dir / 'submission.json').exists():
    ...
    def start(self, request):
        ...
        request = {**request, 'model': request.get('model', getattr(self, 'model', 'chronos2')),
                   'cluster_sha256': digest(config)}
        input_stage = stage or request.get('public_input_stage')
        if input_stage:
            request['suite_definition_sha256'] = suite_identity(self.run_dir, request.get('model', 'chronos2'), input_stage)
        for path in (self.run_dir / 'jobs').glob('*/request.json'):
            ...
            if (stage and old.get('stage') == stage and old['code_sha256'] == code_hash(files)
                    ...
                    and old.get('model', 'chronos2') == request.get('model', 'chronos2')
    ...
    def run(self, command: str, timeout_seconds: int = 120, public_input_stage: str = '', model: str = '') -> dict:
        ...
        model = model or getattr(self, 'model', 'chronos2')
        model_spec(model)
        ...
        for path in visible:
            ...
            if result_file.exists():
                ...
                if not job_id and request.get('stage') == 'bringup' and result.get('verdict', {}).get('passed'):
                    baseline = (self.run_dir / 'baselines' / request.get('model', 'chronos2')
                    ...
                    if not baseline.exists():
                        write_json(baseline, {'job': path.parent.name, 'code_sha256': request['code_sha256'],
                                             ...
                                             'model': request.get('model', 'chronos2'), 'suite_sha256': result['verdict']['suite_sha256'],
            ...
            if compact or job_id:
                ...
                item['details'] = ({'tool': 'chronos_log', 'job_id': path.parent.name,
                                   ...
                                   {'tool': 'chronos_status', 'compact': False, 'job_id': path.parent.name,
    ...
    def submit(self, precision: str = 'bf16', model: str = '') -> dict:
        ...
        for path in requests:
            ...
            if (req.get('kind') == 'evaluate' and req.get('stage') == 'bringup'
                    ...
                    and req.get('model', 'chronos2') == model and req.get('precision') == precision
...
def main():
    ...
    if not args.controller:
        ...
        workspace = Path(tempfile.mkdtemp(prefix='chronos2-fresh-'))
        for name in ('CONTRACT.md', 'TT_GUIDE.md', 'TASK.md', 'BENCHMARK.md', 'LESSONS_NLLB.md'):
        ...
        manifest = {'workspace': str(workspace), 'pid': os.getpid(), 'repository_root': str(repository),
                    ...
                    'seed_policy': 'fresh-public-chronos2',
    ...
    if args.model not in config['tt'].get('weights_by_model', {'chronos2': config['tt'].get('weights')}):
    ...
    ray.init(num_cpus=8, include_dashboard=False, _node_ip_address='127.0.0.1',
             runtime_env={'env_vars': {'PYTHONPATH': str(run / 'harness'),
                                      'CHRONOS_REPOSITORY_ROOT': manifest['repository_root'],
    ...
    try:
        ...
        preflight = get(worker_node.chia_remote(str(workspace), str(run), [tools], 0,
            'Infrastructure check only: list files, read CONTRACT.md, confirm chronos_read ../outside is rejected. '
        ...
        earlier = next((root / 'jobs' / study_job for root in history_runs(run)
                        if (root / 'jobs' / study_job / 'result.json').is_file()), None)
        if earlier:
        ...
        while time.monotonic() < end:
            ...
            previous = get(worker_node.chia_remote(str(workspace), str(run), [tools], turn,
                review['decision']['instruction'] + '\nContinue from your saved STATE.md. Call chronos_status(compact=True) before repeating tests.'
        ...
        if (run / 'submission.json').exists():
            ...
            result = get(experiment.chia_remote(str(run), 'final', {'kind':'evaluate','stage':'full',
                'files':submission['files'], 'precision':submission['precision'], 'model':submission.get('model', 'chronos2')}, frozen))
